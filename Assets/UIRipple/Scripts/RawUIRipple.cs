...
public class RawUIRipple : MonoBehaviour
{
	...
	public void CreateRipple(Vector2 Position)
	{
		...
		{ ThisRipple.transform.position = (Vector2) Camera.main.ScreenToWorldPoint(Position); }
	...
	public bool IsOnUIElement(Vector2 Position)
	{
		...
		{
			var cor = Camera.main.WorldToScreenPoint(corners[i]);
			if (cor.x < bottomLeft.x || cor.y < bottomLeft.y) bottomLeft = new Vector2(cor.x, cor.y);
			if (cor.x > topRight.x || cor.y > topRight.y) topRight = new Vector2(cor.x, cor.y);
		}
		Rect r = new Rect(bottomLeft.x, bottomLeft.y,
			topRight.x - bottomLeft.x, topRight.y - bottomLeft.y);
		Debug.Log(r +" --- " + Position + " -- " + r.Contains(Position));
		if (r.Contains(Position)
			//GetComponent<RectTransform>().rect.Contains(Position)
